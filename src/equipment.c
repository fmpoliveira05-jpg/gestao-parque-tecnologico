...
void readEquipmentMaintenance(Equipment *equipment, int movementNumber) {
    ...
    equipment->maintenanceHistory.maintenance[equipment->maintenanceHistory.maintenanceCount].date = readDate("maintenance");
...
int insert1Equipment(UserList *users, EquipmentList *equipment) {
    ...
    if (searchEquipmentFromName(*equipment, name) == -1) {
        ...
        for (i = 0; i < CATEGORY_OPTIONS; i++) {
            printf("%d - %s\n", i + 1, EQUIPMENT_CATEGORIES[i]);
        }
        i = getInt(1, CATEGORY_OPTIONS, MSG_GET_OPTION);
        strcpy(equipment->equipment[equipment->equipmentCount].category, EQUIPMENT_CATEGORIES[i - 1]);
        equipment->equipment[equipment->equipmentCount].date = readDate("acquisition");
...
void insertEquipmentMaintenance(EquipmentList *equipment) {
    ...
    /* Grow the history of the selected equipment (previously only the first one was ever expanded,
     * which caused a buffer overflow when any other equipment reached its capacity). */
    if (equipment->equipment[pos].maintenanceHistory.maintenanceCount >= equipment->equipment[pos].maintenanceHistory.maintenanceSize) {
        expandMaintenance(&equipment->equipment[pos]);
    }
